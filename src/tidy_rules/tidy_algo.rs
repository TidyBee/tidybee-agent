...
impl TidyAlgo {
    ...
    fn load_rule_from_hashmap(&mut self, table: HashMap<String, Value>) -> Result<String, Box<dyn Error>>  {
...
fn get_string_from_table_safe(table: &HashMap<String, Value>, key: &'static str) -> Result<String, ConfigError> {
    match table.get(key) {
        Some(value) => {
            Ok(value.to_string().clone())
        },
        None => Err(ConfigError::NotFound(format!("Error while getting the key {}", key)))
    }
}
...
mod tests {
    ...
    fn load_rules_from_file_basic() {
        ...
        assert_eq!(tidy_algo.load_rules_from_file(&my_files, rules_path).unwrap(), 3);
    }
    ...
    fn load_rules_from_file_failing() {
        ...
        let my_files = MyFilesBuilder::new()
            ...
            .build()
            .unwrap();
        assert_eq!(tidy_algo.load_rules_from_file(&my_files, rules_path).unwrap(), 0);
    }
}
